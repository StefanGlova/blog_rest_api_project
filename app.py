...
SWAGGER_BLUEPRINT = get_swaggerui_blueprint(
    SWAGGER_URL, API_URL, config={"app_name": "blog_posts"}
)
...
def add():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            new_blog = BlogPost(
                userID=form.userID.data, title=form.title.data, body=form.body.data
            )
            ...
            if form.validate_on_submit():
                ...
                if blog:
                    ...
                    return render_template(
                        "success.html", message="Blog deleted successfully"
                    )
                else:
                    raise TypeError
        except TypeError:
            ...
            if blog:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        return render_template(
                            "error.html", error="Blog not found even externally"
                        )
                except requests.exceptions.RequestException:
                    return render_template(
                        "error.html", error="Failed to get blog from external API"
                    )
    else:
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            if blog:
                ...
                db.session.commit()
                return render_template(
                    "success.html", message="Title updated successfully"
                )
            else:
                return render_template(
                    "error.html", error="Blog not found - title can't be changed"
                )
    else:
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            if blog:
                ...
                db.session.commit()
                return render_template(
                    "success.html", message="Blog text updated successfully"
                )
            else:
                return render_template(
                    "error.html", error="Blog not found - text can't be changed"
                )
    else:
